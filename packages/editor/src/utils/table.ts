  ...
  (type: "row" | "column") => (index: number) => (tr: Transaction) => {
    ...
    if (table) {
      ...
      if (index >= 0 && index < (isRowSelection ? map.height : map.width)) {
        ...
        const cellInFirstRow = cellsInFirstRow[0];
        const cellInLastRow = cellsInLastRow[cellsInLastRow.length - 1];
        if (cellInFirstRow && cellInLastRow) {
          const head = table.start + cellInFirstRow;
          const anchor = table.start + cellInLastRow;
...
export const selectTable = (tr: Transaction) => {
  ...
  if (table) {
    ...
    if (map && map.length) {
      const firstMap = map[0];
      const lastMap = map[map.length - 1];
      if (firstMap && lastMap) {
        const head = table.start + firstMap;
        const anchor = table.start + lastMap;
        const $head = tr.doc.resolve(head);
        const $anchor = tr.doc.resolve(anchor);
